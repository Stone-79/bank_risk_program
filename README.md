...
- 同时训练逻辑回归、随机森林、ExtraTrees、GradientBoosting、AdaBoost 五个模型，并允许用户在前端选择不同模型输出基础违约概率。
...
6. 分别训练逻辑回归、随机森林、ExtraTrees、GradientBoosting、AdaBoost 模型。
7. 计算测试集 AUC。
...
当前方案是“多模型基础概率 + 业务规则修正”的组合模型。系统会同时训练多个机器学习模型，前端默认选择 AUC 较高的 GradientBoosting 模型，也允许用户切换其他模型查看对应预测结果；业务规则负责让新增前端输入直接影响最终违约概率、信用评分和风险等级。该方式适合课程项目展示，也方便解释每类输入对风险结果的影响。
